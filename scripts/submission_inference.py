...
def _gdal_env_options(config: dict[str, Any]) -> dict[str, Any]:
    """GDAL/rasterio options for many small local window reads.

    The platform input puts thousands of TIFFs in one directory. Disabling
    sibling-file directory scans on each open is usually the biggest safe win.
    """
    ...
    overrides = config.get("gdal_env", {})
    if overrides is None:
        return {}
    if not isinstance(overrides, dict):
        raise ValueError("gdal_env must be an object/dict")
    defaults.update({str(key): value for key, value in overrides.items()})
    return defaults
...
def _extract_arrays_from_report(report: dict[str, Any]) -> dict[str, np.ndarray]:
    arrays = report.pop("_arrays", None)
    if arrays is None:
        raise RuntimeError("build_patch_dataset did not return in-memory arrays")
    return {str(name): value for name, value in arrays.items() if isinstance(value, np.ndarray)}
...
def run_inference(config: dict[str, Any]) -> dict[str, Any]:
    ...
    use_in_memory_patches = bool(config.get("use_in_memory_patches", False))
    write_patch_npz = bool(config.get("write_patch_npz", not use_in_memory_patches))
    compressed_patch_npz = bool(config.get("compressed_patch_npz", True))
    ...
    with _patch_dataset_context(config):
        report = build_patch_dataset(
            ...
            write_reports=False,
            write_npz=write_patch_npz,
            compressed_npz=compressed_patch_npz,
            return_arrays=use_in_memory_patches,
            fast_inventory=bool(config.get("fast_inventory", False)),
    ...
    timings["model_load_seconds"] = time.perf_counter() - model_load_start
    ...
    if use_in_memory_patches:
        arrays = _extract_arrays_from_report(report)
    else:
        with np.load(test_npz, allow_pickle=False) as npz:
            arrays = {name: npz[name] for name in npz.files}
    query_rows = read_query_rows(points_csv, arrays)
    timings["npz_and_query_rows_seconds"] = time.perf_counter() - data_load_start
    ...
    for start in range(0, len(query_rows), batch_size):
        ...
        with torch.no_grad():
            ...
            for em in ensemble_models:
                ...
                crop_outputs["crop_logits"] = crop_outputs["crop_logits"] + em_out["crop_logits"]
                stage_outputs["stage_logits"] = stage_outputs["stage_logits"] + em_out["stage_logits"]
            synchronize_device(device)
            ...
            logits_to_cpu_seconds += time.perf_counter() - logits_start
        crop_logit_chunks.append(crop_logits_np)
        stage_logit_chunks.append(stage_logits_np)
    timings["batch_prepare_seconds"] = batch_prepare_seconds
    timings["model_forward_seconds"] = forward_seconds
    timings["logits_to_cpu_seconds"] = logits_to_cpu_seconds
    ...
    timing_rows = max(int(len(query_rows)), 1)
    timings["queries_per_total_second"] = timing_rows / timings["total_seconds"] if timings["total_seconds"] > 0 else 0.0
    timings["queries_per_forward_second"] = timing_rows / timings["model_forward_seconds"] if timings["model_forward_seconds"] > 0 else 0.0
    return {
        ...
        "use_in_memory_patches": bool(use_in_memory_patches),
        "write_patch_npz": bool(write_patch_npz),
        "compressed_patch_npz": bool(compressed_patch_npz),
        "fast_inventory": bool(config.get("fast_inventory", False)),
        "batch_raster_reads": bool(config.get("batch_raster_reads", False)),
        "max_batch_union_pixels": int(config.get("max_batch_union_pixels", 262144)),
        "max_batch_union_overread_ratio": float(config.get("max_batch_union_overread_ratio", 6.0)),

...
def build_fast_file_index(tiff_dirs: list[Path], root: Path, band_order: list[str] | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Build minimal audit counts and selected-file index for inference.

    This keeps the same valid-file selection rule as audit_tiff_files +
    select_file_index, but avoids constructing the full report dataframe when
    submission inference only needs status counts and selected paths.
    """
    ...
    for path in paths:
        entry = {
            ...
            "folder": path.parent.name,
            "name": path.name,
            "canonical_name": canonical_name(path.name),
            "parsed": parse_tiff_name(path.name),
        }
        by_canonical[(entry["folder"], entry["canonical_name"])].append(len(entries))
        entries.append(entry)

    duplicate_of: dict[int, Path] = {}
    for group_indices in by_canonical.values():
        exact = [idx for idx in group_indices if entries[idx]["name"] == entries[idx]["canonical_name"]]
        keeper_idx = exact[0] if exact else sorted(group_indices, key=lambda idx: str(entries[idx]["path"]))[0]
        for idx in group_indices:
            if idx != keeper_idx:
                duplicate_of[idx] = entries[keeper_idx]["path"]

    status_rows: list[dict] = []
    selected_groups: dict[tuple[str, str, str], list[dict]] = defaultdict(list)
    for idx, entry in enumerate(entries):
        ...
        if parsed is None:
            ...
            status = "unsupported_band"
        elif idx in duplicate_of:
            status = "duplicate_canonical"
        else:
            status = "valid"
            selected_groups[(parsed.region_id, parsed.start_norm, parsed.band_id)].append(
                {
                    "path": str(entry["rel_path"]),
                    "level": str(parsed.level),
                    "level_rank": level_rank(str(parsed.level)),
                }
            )
        status_rows.append({"status": status})

    selected_rows: list[dict] = []
    for (region_id, start_norm, band_id), candidates in sorted(selected_groups.items()):
        candidates = sorted(candidates, key=lambda item: (item["level_rank"], item["path"]))
        best = candidates[0]
        selected_rows.append(
            {
                ...
                "candidate_levels": "|".join(sorted({item["level"] for item in candidates})),
                "candidate_paths": "|".join(item["path"] for item in candidates),
                "selection_reason": "prefer_L2A_then_path",
            }
        )

    file_df = pd.DataFrame(status_rows)
    selected_df = pd.DataFrame(selected_rows).sort_values(["region_id", "start_norm", "band_id"]).reset_index(drop=True)
    if selected_df.empty:
        if not paths:
            raise ValueError("no TIFF files found")
        raise ValueError("no valid Sentinel-2 band TIFFs found after filename audit")
    return file_df, selected_df


def select_file_index(file_df: pd.DataFrame) -> pd.DataFrame:
    if file_df.empty:
        raise ValueError("no TIFF files found")
    valid = file_df[file_df["status"] == "valid"]
    if valid.empty:
        raise ValueError("no valid Sentinel-2 band TIFFs found after filename audit")

    groups: dict[tuple[str, str, str], list[dict]] = defaultdict(list)
    for row in valid.itertuples(index=False):
        groups[(str(row.region_id), str(row.start_norm), str(row.band_id))].append(
            {
                "path": str(row.path),
                "level": str(row.level),
                "level_rank": level_rank(str(row.level)),
            }
        )

    rows: list[dict] = []
    for (region_id, start_norm, band_id), candidates in sorted(groups.items()):
        candidates = sorted(candidates, key=lambda item: (item["level_rank"], item["path"]))
        best = candidates[0]
        rows.append(
            {
